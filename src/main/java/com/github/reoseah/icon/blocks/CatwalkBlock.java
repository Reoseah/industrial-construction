...
public class CatwalkBlock extends Block implements Waterloggable, WrenchableBlock {
        ...
        if (world.getBlockState(pos.up()).isAir()) {
            for (Direction dir : Direction.Type.HORIZONTAL) {
                ...
                if (upOffsetBlock == this || upOffsetBlock instanceof CatwalkConnectingBlock && ((CatwalkConnectingBlock) upOffsetBlock).shouldCatwalkConnect(upOffsetState, world, upOffset, dir)) {
                    return ICon.Blocks.CATWALK_STAIRS.getDefaultState().with(CatwalkStairsBlock.FACING, dir.getOpposite());
                }
            }
        }
    ...
    public boolean hasBorder(BlockView world, BlockPos pos, Direction side) {
        ...
        if (block instanceof CatwalkBlock || block == ICon.Blocks.SCAFFOLDING || block == ICon.Blocks.CONDUIT_IN_SCAFFOLDING || block == Blocks.CAULDRON) {
            return false;
        }
